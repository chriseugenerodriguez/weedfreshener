...
<script>
	...
	})(window,document,'script','//www.google-analytics.com/analytics.js','<?php echo $this->get_ga_function_name(); ?>');
	<?php echo $this->get_ga_function_name(); ?>( 'create', '<?php echo esc_js( $this->get_tracking_id() ); ?>', 'auto' );
...
<!-- end WooCommerce Google Analytics Pro -->
	 ...
	 *
	 * @since 1.0.0
	 * @return string the visitor's ID from Google's cookie, or generated
	 */
	 ...
	 *
	 * Returns 1 or 2 identities - the CID (GA unique ID from cookie) and
	...
	private function do_not_track( $admin_event = false, $user_id = null ) {
		...
		// track, unless disabled for the current user either by MonsterInsights or by us.
	 ...
	 * @param bool $admin_event (optional) whether this is an admin event
	 * @return bool
	 */
	private function is_tracking_enabled_for_user_role( $user_id = null, $admin_event = false ) {
		...
		if ( ! isset( $this->user_tracking_enabled[ $user_id ] ) ) {
			...
			if ( $user && wc_google_analytics_pro()->is_monsterinsights_active() ) {
				...
				$enabled = $admin_event ? $admin_event : ( 'yes' === $this->get_option( 'admin_tracking_enabled' ) );
	 ...
	 *
	 * Prevents duplication of tracking events when user reloads page or submits
	...
	public function signed_in( $user_login, $user ) {
		 ...
		 */
		if ( in_array( $user->roles[0], apply_filters( 'wc_google_analytics_pro_signed_in_user_roles', array( 'subscriber', 'customer' ) ), true ) ) {
	...
	public function selected_payment_method() {
		...
		$js = sprintf( "$( 'form.checkout' ).on( 'click', 'input[name=\"payment_method\"]', function() { var args = { payment_method: wc_ga_pro.get_payment_method_title( this.value ) }; wc_ga_pro.payment_method_tracked = true; %s });", $handler_js );
	 ...
	 *
	 * This method uses the ecommerce.js plug-in to send data to GA.
	 *
	 * @since 1.0.0
	 * @param int $order_id the order ID
	 */
	 ...
	 *
	 * Triggered for orders that were previously on-hold, like BACS or cheque
	 * orders.
	 *
	 * @since 1.0.0
	 * @param int $order_id the order ID
	...
	public function completed_payment( $order_id ) {
		...
		// don't track order when its already tracked or "paid" - note that this uses a different meta key from completed purchase on purpose :)
		if ( $order->is_paid() || 'yes' === get_post_meta( $order_id, '_wc_google_analytics_pro_tracked_completed_payment', true ) ) {
			return;
		}
		...
		// mark order as tracked
		update_post_meta( SV_WC_Order_Compatibility::get_prop( $order, 'id' ), '_wc_google_analytics_pro_tracked_completed_payment', 'yes' );
	}
