	...
	public function get_condition_resolution( $data, $args ) {
		...
		if ( $data[ 'modifier' ] === 'in' && ! empty( $qualifying_products ) ) {
			...
			if ( count( $qualifying_products ) == 1 ) {
				...
				return sprintf( _n( 'remove &quot;%s&quot; from your cart', 'remove &quot;%s&quot; from this shipment', $package_count, 'woocommerce-conditional-shipping-and-payments' ), current( $qualifying_products ) );
				...
				for ( $i = 1; $i < count( $qualifying_products ) - 1; $i++ ) {
					...
					$string = sprintf( __( '%1$s, &quot;%2$s&quot;', 'woocommerce-conditional-shipping-and-payments' ), $string, $qualifying_products[ $i ] );
				}

				/* translators: Used to stitch together product names - last name */
				$string = sprintf( __( '%1$s, and &quot;%2$s&quot;', 'woocommerce-conditional-shipping-and-payments' ), $string, end( $qualifying_products ) );
