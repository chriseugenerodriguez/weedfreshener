...
class AV8_Cart_Receipt {
	...
	public function set_products($woocommerce) {
		...
		foreach ($keys as $key) {
			...
			if(isset($cart_contents[$key]['variation_id'])) {
				$products[] = array('product_id' => $cart_contents[$key]['product_id'], 'variation_id' => $cart_contents[$key]['variation_id'], 'quantity' => $cart_contents[$key]['quantity']);		
			} elseif(isset($cart_contents[$key]['product_id'])) {
				$products[] = array('product_id' => $cart_contents[$key]['product_id'], 'quantity' => $cart_contents[$key]['quantity']);
			}
		}
	...
	public function status() {
		...
		$since_connected = time() - $this->last_connected();
...
public function email() {
	...
	public function insert_receipt() {
		...
		update_post_meta( $this->post_id, 'av8_cart_created', time() ); //Save created time
		update_post_meta( $this->post_id, 'av8_last_updated', time() ); //Initialize the updated time
	}
