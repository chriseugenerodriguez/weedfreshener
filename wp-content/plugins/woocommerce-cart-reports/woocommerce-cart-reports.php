...
/*  Copyright 2016  WPBackOffice  (email : support@wpbackoffice.com)
...
if ( is_woocommerce_active() ) {
	...
	*/
	...
	function dashboard_stats_where_abandoned_range( $where ) {
		...
		$where .= " AND post_modified > '" . date('Y-m-d G:i:s',  time() +($offset*3600) - ($woocommerce_cart_reports_options['dashboardrange'] * 24 * 60 *60 )) ."' and post_modified < '" . date('Y-m-d G:i:s',  time() +($offset*3600)  - $woocommerce_cart_reports_options['timeout']) . "' ";
		return $where;
	}
	...
	function dashboard_stats_where_abandoned_lifetime( $where ) {
		...
		if(WP_DEBUG == true)
			assert(is_numeric($offset));
		$where .= " AND post_modified < '" . date('Y-m-d G:i:s',  time() +($offset*3600)  - $woocommerce_cart_reports_options['timeout']) . "' ";
		return $where;
	}
	/*
	...
	function dashboard_stats_where_converted( $where ) {
		...
		if(WP_DEBUG == true)
			assert(is_numeric($offset));
		$where .= " AND post_modified > '" . date('Y-m-d G:i:s',  time() + ($offset*3600) - ($woocommerce_cart_reports_options['dashboardrange'] * 60*60 * 24)) . "' ";
		return $where;
	}
...
' . "href='?timestamp=" . $timestamp . "&_wpnonce=" . wp_create_nonce('trash-the-carts') . "&cart-action=clear'>Clear Carts</a>";
	$links[] =  $settings_link; 
	return $links; 
}
...
function clear_all_carts() {
	if(isset($_GET['timestamp']) && $_GET['timestamp'] && isset($_GET['cart-action']) && $_GET['cart-action'] == 'clear' && current_user_can('delete_plugins') && wp_verify_nonce($_REQUEST['_wpnonce'], 'trash-the-carts')):
		$current_stamp = time();
