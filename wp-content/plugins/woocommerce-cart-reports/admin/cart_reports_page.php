...
class AV8_Cart_Reports_Page {
 ...
 function get_report_cart_reports( $name ) {
	...
	if ( defined( 'WOOCOMMERCE_VERSION' ) && version_compare( WOOCOMMERCE_VERSION, '2.3', '>=' ) ) {
		...
		include_once( 'reports/class-wc-report-' . $name . '-legacy.php' );		
	}
...
function woocommerce_product_abandoned() {
	...
	if ( $chosen_product_ids && is_array( $chosen_product_ids ) ) {
		...
		<h4><?php printf( __( 'Abandoned Carts containing %s:', 'woocommerce_cart_reports' ), implode( ', ', $chosen_product_titles ) ); ?></h4>
		<table class="bar_chart">
			<thead>
				<tr>
				...
				<?php
					...
					endforeach; else echo '<tr><td colspan="3">'.__('No Carts :)', 'woocommerce_cart_reports').'</td></tr>';
				?>
			</tbody>
		</table>
		<?php
