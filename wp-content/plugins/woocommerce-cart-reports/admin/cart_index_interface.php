...
public function restrict_manage_posts() {
    ...
    if(($pagenow == 'edit.php') && isset($_GET['post_type']) && ($_GET['post_type'] == 'carts'))
    {
		...
		if(isset($_GET['lifetime']) || !isset($_GET['mv']))
		{
			...
			$start_date = strtotime($post->post_modified); // Add on a day for good measure.
		}
...
if($pagenow == 'edit.php' && isset($_GET['post_type']) && $_GET['post_type'] == 'carts'){
	...
	global $offset;
		...
		if(isset($_GET['lifetime']) || !isset($_GET['mv']))
		{
			...
			if(isset($post[0]))
				$post = $post[0];
			if(isset($post) && sizeof($post) > 0)			
			$start_date = strtotime($post->post_modified) - (86400); // Add on a day for good measure.
		}
	...
	if(isset($_GET['mv'])) {
		if($_GET['mv'] == "Open")
			$where .= " AND post_modified > '".date('Y-m-d G:i:s', time() + ($offset * 3600) - $timeout)."'";
