				 ...
				 $loop = 0; if (sizeof($order_items)>0) foreach ($order_items as $item) : 
					...
					<tr class="item <?php echo $table_color; ?>" rel="<?php echo $loop; ?>">
						<td class="thumb" width="20%">
							<a href="<?php echo esc_url( admin_url('post.php?post='. $_product->id .'&action=edit') ); ?>" class="help_tip cart-product-thumbnail" data-tip="<?php
								echo '<strong>'.__('Product ID:', 'woocommerce_cart_reports').'</strong> '. $_product->id;
								...
								echo '<br/><strong>'.__('Product SKU:', 'woocommerce_cart_reports').'</strong> '; if ($_product->sku) echo $_product->sku; else echo '-';
						...
						<td class="name" width="20%">
							...
							<a href="<?php echo esc_url( admin_url('post.php?post='. $_product->id .'&action=edit') ); ?>"><strong><?php echo $_product->get_title(); ?></strong></a>
							<?php
								if (isset($_product->variation_data)) echo '<br/>' . wc_get_formatted_variation( $_product->variation_data, true );
							?>
