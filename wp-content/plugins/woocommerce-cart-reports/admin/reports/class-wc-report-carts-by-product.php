...
class WC_Report_Carts_By_Product extends WC_Admin_Report {
	...
	public function get_chart_legend() {
		...
		if ( $order_items ) {
			...
			$order_item_counts  = $this->prepare_chart_data( $product_objs, 'post_modified', 'order_item_count', $this->chart_interval, $this->start_date, $this->chart_groupby );
		...
		$total_items    = $count;
		$legend[] = array(
			'title' => sprintf( __( '%s Product Abandonments', 'woocommerce_cart_reports' ), '<strong>' . $total_items  . '</strong>' ),
	...
	public function current_filters() {
		...
		echo '<p><a class="button" href="' . esc_url( remove_query_arg( 'product_ids' ) ) . '">' . __( 'Reset', 'woocommerce_cart_reports' ) . '</a></p>';
	}
	...
	public function products_widget() {
		?>
		<h4 class="section_title"><span><?php _e( 'Product Search', 'woocommerce_cart_reports' ); ?></span></h4>
		<div class="section">
			<form method="GET">
				<div>
	...
	public function get_main_chart() {
		...
		if ( ! $this->product_ids ) {
			...
			<div class="chart-container">
				<p class="chart-prompt"><?php _e( '&larr; Choose a product to view stats', 'woocommerce_cart_reports' ); ?></p>
			</div>
			<?php
		} else {
		...
		if ( $order_items ) {
			...
			//$order_item_counts  = $this->prepare_chart_data( $product_sales, 'post_modified', 'order_item_count', $this->chart_interval, $this->start_date, $this->chart_groupby );
			...
			<script type="text/javascript">
				...
				jQuery(function(){
					...
					var drawGraph = function( highlight ) {
						...
						var series = [
							{
								label: "<?php echo esc_js( __( 'Number of items Abandoned', 'woocommerce_cart_reports' ) ) ?>",
